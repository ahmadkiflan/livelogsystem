...
<div class="relative p-4 bg-white rounded-lg border dark:bg-gray-800 sm:p-5">
    ...
    <!-- Modal body -->
    <form action="{{ route('dashboard.update', $post->slug) }}" method="POST">
        @csrf
        @method('PATCH')
        <div class="">
            <div class="mb-4">
                <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Title</label>
                <input type="text" name="title" id="title" value="{{ old($post->title, $post->title) }}"
            ...
            <div class="mb-4">
                ...
                <textarea id="content" rows="4" name="content"
                    ...
                    placeholder="Write post content here">{{ old($post->content, $post->content) }}</textarea>
            </div>
        </div>
